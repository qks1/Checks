...
MainWindow::MainWindow(bool success, QSqlDatabase db)
{
    is_connect = success;
    base = db;
    settings = new QSettings("erk", "Checks");
    docs_list = 0;
    doc = 0;
    mwidget = 0;
...
void MainWindow::create_central_widget(){
    ...
    // по двойному щелчку на таблице - показать окно документа
    connect(this->docs_list, SIGNAL(show_doc_signal(int)),
            this, SLOT(signal_from_list(int)));
}
...
void MainWindow::signal_from_list(int id){
